    ...
    <div className={`pumpkin-wave-overlay phase-${phase}`}>
      <div className="pumpkin-wave-particles">
        {pumpkins.map((pumpkin) => {
          const top = (animatePumpkins ? pumpkin.endY : pumpkin.startY).toFixed(
            2,
          );
          const left = (
            animatePumpkins ? pumpkin.endX : pumpkin.startX
          ).toFixed(2);
          const size = (
          ...
          const transition =
            `top ${MOVE_DURATION_SECONDS}s cubic-bezier(0.33, 1, 0.68, 1) ${pumpkin.delay}s, ` +
